...
def extract_features(model, data_loader):
    ...
    with torch.no_grad():
        ...
        for i, (img_transformed, fpath, pid, camid, domain) in enumerate(data_loader):
            fnames = fpath
            pids = pid
            cids = camid
...
def initial_classifier(model, data_loader):
    ...
    # some protection jobs
    in_feats = None
    try:
        in_feats = model.module.classifier.in_features
    except Exception:
        try:
            in_feats = model.classifier.in_features
        except Exception:
            # fallback to base.in_planes if available
            try:
                in_feats = model.module.base.in_planes
            except Exception:
                try:
                    in_feats = model.base.in_planes
                except Exception:
                    in_feats = class_centers.size(1)

    if class_centers.dim() == 2 and class_centers.size(1) != in_feats:
        if class_centers.size(1) > in_feats:
            class_centers = class_centers[:, :in_feats]
        else:
            pad = class_centers.new_zeros(class_centers.size(0), in_feats - class_centers.size(1))
            class_centers = torch.cat([class_centers, pad], dim=1)
