...
def fast_test_p_s(model, all_train_sets, all_test_only_sets,set_index, args, logger=None,writer=None, return_avg=False):
    ...
    for temp_loaders in loaders:
        dataset, num_classes, clip_stage1_loader, clip_stage2_loader, BLIP_loader, test_loader, init_loader, name=temp_loaders
        query_features_meter, query_pids_meter, query_cids_meter = CatMeter(), CatMeter(), CatMeter()
        gallery_features_meter, gallery_pids_meter, gallery_cids_meter = CatMeter(), CatMeter(), CatMeter()
        ...
        torch.cuda.empty_cache()
        print(time_now(), f' {name} feature start ')
        with torch.no_grad():
            for i, info in enumerate(tqdm(test_loader)):
                img_transformed, fpaths,targets, cids, domains=info
                images, pids, cids=img_transformed,targets,cids
                images = images.cuda()
                eval_choice = 'EO' 
                if eval_choice == 'EO':
                    output = model(images)
                    combined = F.normalize(output,dim=1)
                elif eval_choice == 'EOQ':
                    output = model(images, requires_region_attn=True)
                    global_feat, region = output
                    region_flat = region.reshape(region.size(0), -1)
                    combined = torch.cat([global_feat, region_flat], dim=1)
                    combined = F.normalize(combined, dim=1)
                elif eval_choice == 'Q':
                    output = model(images, requires_region_attn=True)
                    global_feat, region = output
                    region_flat = region.reshape(region.size(0), -1)
                    combined = F.normalize(region_flat, dim=1)

                features_meter.update(combined.data)
        ...
        print(time_now(), f' {name} feature done')
        torch.cuda.empty_cache()

        rank1, map = _cmc_map(query_features_meter, gallery_features_meter)
        print("mAP/Rank1:\t{:.1f}/{:.1f}".format(map, rank1))

        results_dict[f'{name}_mAP'], results_dict[f'{name}_Rank1'] = map, rank1

        if writer is not None:
            writer.add_scalar(tag="results/{}_mAP".format(name), scalar_value=map,
                               global_step=set_index)
            writer.add_scalar(tag="results/{}_R@1".format(name), scalar_value=rank1,
                              global_step=set_index)

    results_str = ''
    for criterion, value in results_dict.items():
        results_str = results_str + f'\n{criterion}: {value}'
    aver_seen_mAP, aver_seen_r1, aver_unseen_mAP, aver_unseen_r1=print_results(args,results_dict, all_train_sets, all_test_only_sets, set_index, logger=logger)
...
def print_results(args, rank_map_dict, all_train_sets, all_test_only_sets,set_index, logger=None):
    seen_r1 = []
    seen_map = []
    unseen_r1 = []
    unseen_map = []
