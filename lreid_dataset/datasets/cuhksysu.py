...
class CUHKSYSU(ImageDataset):
    ...
    def process_dir(self, dir_path, relabel=False):
        img_paths = glob.glob(osp.join(dir_path, '*.jpg'))
        img_paths.sort()
        pattern = re.compile(r'([-\d]+)_s([-\d]+)_([-\d]+)_([-\d]+)') # 规定捕捉的内容格式
    ...
    def preprocessing(self):
        ...
        train = Train_mat['Train'].squeeze()
        # loadmat()加载.mat后返回字典，和json.load()类似
        # Train.mat包含 '__header__' 文件头信息；'__version__' MATLAB版本； '__globals__' 全局变量；'Train' 我们需要的信息
        # Train_mat['Train'].shape = (5532, 1) 有5532个ID进行训练
        # Train_mat['Train'][0,0] -> 获取第一个ID的数据
        # train.shape (5532, 1) -> 5532
        n_train = 0
        for index, item in enumerate(train):
            # 每个item是一个MATLAB struct, 包含该人员的所有信息
            pid_name = item[0, 0][0][0]
            pid = int(pid_name[1:])
            id_name_to_pid[pid_name] = pid
            scenes = item[0, 0][2].squeeze()
