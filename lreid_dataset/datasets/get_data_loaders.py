...
def get_data(name, data_dir, height, width, batch_size, workers, num_instances, select_num=0):
    ...
    clipreid_stage1_transformer = T.Compose([
        T.Resize((height, width), interpolation=3),
        T.ToTensor(),
        normalizer
    ])

    clipreid_stage2_transformer = T.Compose([
        T.Resize((height, width), interpolation=3),
        ...
        T.RandomErasing(probability=0.3, mean=[0.485, 0.456, 0.406]) 
    ])

    train_transformer_strong = T.Compose([
        T.Resize((height, width), interpolation=3),
        T.RandomHorizontalFlip(),
        T.RandomCrop(size=(height, width), padding=int(width*0.125), padding_mode='reflect'),
        RandAugmentMC(n=2, m=10),
        T.ToTensor(),
        normalizer,
    ])

    BLIP_transformer = T.Compose([
        T.Resize((height, width), interpolation=3),
        T.ToTensor(),
        normalizer
    ])
    ...
    if rmgs_flag:
        ...
        sampler = None
    clip_stage1_loader = IterLoader(
        DataLoader(Preprocessor(train_set, root=dataset.images_dir,transform=clipreid_stage1_transformer),
                   batch_size=batch_size, num_workers=workers, sampler=sampler,
                   shuffle=not rmgs_flag, pin_memory=True, drop_last=True), length=iters)

    clip_stage2_loader = IterLoader(
        DataLoader(Preprocessor(train_set, root=dataset.images_dir,transform=clipreid_stage2_transformer,  strong_transform=train_transformer_strong),
                   batch_size=batch_size, num_workers=workers, sampler=sampler,
                   shuffle=not rmgs_flag, pin_memory=True, drop_last=True), length=iters)
    
    BLIP_loader = DataLoader(Preprocessor(train_set, root=dataset.images_dir,transform=BLIP_transformer),
    ...
    return [dataset, num_classes, clip_stage1_loader, clip_stage2_loader, BLIP_loader, test_loader, init_loader, name]
