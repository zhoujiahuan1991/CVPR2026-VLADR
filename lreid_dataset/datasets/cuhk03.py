...
class CUHK03(ImageDataset):
    ...
    def preprocess_split(self):
        ...
        def _process_images(img_refs, campid, pid, save_dir):
            # 用于从mat格式中恢复出png
            # 返回复原的图像路径（e.g., '/data/cuhk03/images_detected/3_016_1_01.png'）
            # '/data/cuhk03/images_detected/3_016_1_01.png', 视角1，第一张，相机对编号为3，人员ID16
            img_paths = [] # 存储生成的图片路径
            for imgid, img_ref in enumerate(img_refs):
                img = _deref(img_ref)
                # _deref() 从mat文件中提取出了实际图像矩阵
                # img现在是一个numpy数组，e.g., img.shape = (160, 60, 3)
                if img.size == 0 or img.ndim < 3:
                    continue # skip empty cell， 从而确保是有效的RGB图像
                # images are saved with the following format, index-1 (ensure uniqueness)
                # campid: index of camera pair (1-5)
                # pid: index of person in 'campid'-th camera pair
                # viewid: index of view, {1, 2}
                ...
                viewid = 1 if imgid < 5 else 2
                img_name = '{:01d}_{:03d}_{:01d}_{:02d}.png'.format(
                    campid + 1, pid + 1, viewid, imgid + 1 # 相机对编号，人员ID，视角ID，图片序号
                )
                img_path = osp.join(save_dir, img_name)
                if not osp.isfile(img_path):
                    imageio.imwrite(img_path, img) # 将numpy数组保存为PNG文件
                img_paths.append(img_path)
            return img_paths

        def _extract_img(image_type):
            # 输入图像类型，返回所有人员的元数据列表，（campid, pid, img_path_list）
            '''
            meta_data = [
                # 相机对1的所有人员
                (1, 1, ['1_001_1_01.png', '1_001_1_02.png', ...]),
                (1, 2, ['1_002_1_01.png', '1_002_2_06.png', ...]),
                ...
                (1, 312, [...]),
            ...
            meta_data = []
            imgs_dir = self.imgs_detected_dir if image_type == 'detected' else self.imgs_labeled_dir
            for campid, camp_ref in enumerate(mat[image_type][0]): # 外层遍历相机对
                camp = _deref(camp_ref)
                num_pids = camp.shape[0]
                for pid in range(num_pids): # 内层遍历每个相机对中的人员
                    img_paths = _process_images(
                        camp[pid, :], campid, pid, imgs_dir
                    )
                    assert len(img_paths) > 0, \
