...
class CUHK02(ImageDataset):
    ...
    CUHK02 前3位用来标注不同视角下的相同ID
    最后返回的数据集格式是(image_path, pid, camid, 11)
    """
    dataset_dir = 'cuhk02'
    cam_pairs = ['P1', 'P2', 'P3', 'P4', 'P5']
    test_cam_pair = 'P5'
    ...
    def __init__(self, root='', **kwargs):
        ...
        self.check_before_run(required_files)# check before going deeper
    ...
    def get_data_list(self):
        ...
        for cam_pair in self.cam_pairs:
            ...
            impaths1 = glob.glob(osp.join(cam1_dir, '*.png')) # 通配符，该目录下的所有以.png结尾的文件路径
            impaths2 = glob.glob(osp.join(cam2_dir, '*.png'))
            ...
            if cam_pair == self.test_cam_pair:
                ...
                for impath in impaths1:
                    pid = osp.basename(impath).split('_')[0]
                    pid = int(pid)
                    query.append((impath, pid, camid, 11)) # CUHK02 dstid=11
                camid += 1
                ...
                pids = set(pids1 + pids2) # 去重
                pid2label = {
                    pid: label + num_train_pids 
                    # + num_train_pids是因为如果已有训练集人员num_train_pids各，那么新增人员要在原有基础上进行累计
                    for label, pid in enumerate(pids) # 对字典进行enumerate, 得到label
                }
